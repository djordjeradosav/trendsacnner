  ...
  const { data: prevScores } = useQuery<ScoreRow[]>({
    ...
    queryFn: async () => {
      ...
      const { data } = await supabase
        .from("scores")
        .select("pair_id, score, trend, timeframe, scanned_at, id, ema_score, adx_score, rsi_score, macd_score, news_score, social_score, macro_score, ema20, ema50, ema200, rsi, adx, macd_hist")
